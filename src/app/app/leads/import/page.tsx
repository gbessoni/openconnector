...
export default async function ImportPage() {
  ...
  // Admins can pick from any user; connectors only get themselves
  const users = isAdmin
      ...
      <div className="max-w-5xl mx-auto p-8">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6 text-sm">
          ...
          <code className="block bg-white border border-blue-200 rounded p-2 text-xs text-blue-900 overflow-x-auto">
            ...
            title, vendor, category, why_fit, notes, status
            {isAdmin ? ", owner_email" : ""}
          </code>
          ...
          {isAdmin && (
            <p className="text-blue-800 mt-1">
              <strong>owner_email</strong> (optional): match by email to assign
              to that connector. If empty, the default owner below is used.
            </p>
          )}
        </div>

        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <ImportForm users={users} isAdmin={isAdmin} />
        </div>
      </div>
    </AppShell>
  );
